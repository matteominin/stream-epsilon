...
/**
 * Manager for NodeInstance objects.
 * 
...
public class NodeInstanceManager {
    ...
    public NodeInstance getOrCreate(NodeMetamodel nodeMetamodel) {
        ...
        if (existing.isPresent()) {
            ...
            if (isRunning(existing.get().getId()))
                return existing.get();

            // The existing node is deprecated and it is not in execution
            this.logger.info("A node instance for bode {} was found but is deprecated: deleting it",
                    nodeMetamodel.getId());
    ...
    public NodeInstance getOrCreate(String nodeMetamodelId) {
        ...
        if (existing.isPresent()) {
            logger.info("Found existing NodeInstance for metamodel ID: {}", nodeMetamodelId);
            return existing.get();
        }
        ...
        if (metamodel.isEmpty()) {
            logger.error("Node metamodel not found for ID: {}", nodeMetamodelId);
            throw new RuntimeException("Node metamodel not found");
        }

        logger.info("Creating new NodeInstance for metamodel ID: {}", nodeMetamodelId);
     ...
     * @return Returns true if there is at least one instance of the node that is
     *         being executed
     */
    ...
    public void onMetaNodeUpdated(NodeMetamodelUpdateEvent event) {
        ...
        if (instance.isPresent()) {
            ...
            this.logger.info("Operation layer received metamodel update event: updating node instance for node {}",
                    instance.get().getId());

            // If the node is running then we cannot hot swap NOW, but we re-create the
            // whole node LATER
            if (isRunning(id)) {

                // Re-Installation
                this.logger.info("Node instance {} had a breaking change update: no hot-swap, marking it as deprecated",
                        instance.get().getId());

                // We mark it as deprecated, when the last execution of this node finishes, it
                // will be deleted
            ...
            this.logger
                    .info("Operation layer received metamodel update event but no instance has the updated metamodel");
