...
class IntentDetectionServiceIT {
    ...
    void shouldHandleMultipleIntentsProperly() {
        ...
        // Check variables. At list Paris and Tomorrow should be present among the
        // values.
    ...
    void shouldMapVariablesCorrectlyForAI4NE() {
        ...
        mockIntent.setDescription(
                "Route user requests across different nodes (or cloud resources) according to user ``intent'' and hardware availability,  relying on specialized AI modules to optimize networking decisions. Supporting different request types like streaming, real-time translation, IoT, etc.");
        ...
        assertTrue(result.getUserVariables().values().stream().anyMatch(
                value -> target.stream().anyMatch(t -> value.toString().toLowerCase().contains(t.toLowerCase()))));
